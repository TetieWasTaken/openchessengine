...
describe('Half/Full moves', () => {
	...
	it('Should increment halfmove on knight move', () => {
		...
		makeMove(board, { from: [1, 0], to: [2, 2], piece: { type: Piece.Knight, colour: Colour.Black } });

		expect(board.getHalfmove()).toBe(1);
	});

	it('Should reset halfmove on pawn move', () => {
		const board = new Board('5k2/8/8/8/3PK3/8/8/8 w - - 3 9');
		makeMove(board, { from: [3, 4], to: [3, 3], piece: { type: Piece.Pawn, colour: Colour.White } });

		expect(board.getHalfmove()).toBe(0);
	});

	it('Should increment fullmove on black move', () => {
		const board = new Board();
		makeMove(board, { from: [0, 6], to: [0, 5], piece: { type: Piece.Pawn, colour: Colour.White } });
		makeMove(board, { from: [0, 1], to: [0, 2], piece: { type: Piece.Pawn, colour: Colour.Black } });

		expect(board.getFullmove()).toBe(2);
	});
});
