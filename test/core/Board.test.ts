...
describe('Board generator', () => {
	...
	test('White piece', () => {
		const board = new Board();
		expect(board.getPieceAt(0, 7)?.[1]).toBe(Colour.White);
	});

	test('Black piece', () => {
		const board = new Board();
		expect(board.getPieceAt(0, 0)?.[1]).toBe(Colour.Black);
	});

	test('Custom FEN', () => {
		const board = new Board('8/8/8/8/3k4/8/8/8 w - - 0 1');
		expect(board.getPieceAt(3, 4)?.[0]).toBe(Piece.King);
	});
	...
	test('Custom en passant square', () => {
		...
		expect(board.getEnPassantSquare()).toEqual([4, 5]);
	});
});
