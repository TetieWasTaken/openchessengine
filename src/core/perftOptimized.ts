...
/**
 * Optimized perft function using make/unmake moves instead of board cloning
 */
export function perftOptimized(board: Board, depth: number): number {
	if (depth === 0) {
		return 1;
	}
	...
	for (const move of moves) {
		...
		nodes += perftOptimized(board, depth - 1);
